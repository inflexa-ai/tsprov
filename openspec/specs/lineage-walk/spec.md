# lineage-walk Specification

## Purpose

The directional, bounded, cycle-safe walk over a `ProvGraph` — root normalization
(elements, relations seeding both endpoints, QNames, strings), effect-to-cause direction
semantics with symmetric `alternateOf`, relation profiles + injected edge predicate,
per-direction depth bounds behind a safety ceiling, and the flat honest result
(nodes/edges/frontier/unknownRoots). Stage 3 of the lineage sequence
