# record-resolution Specification

## Purpose

Resolving a caller's partial reference — exact id, identifier fragment, record class,
attribute predicate, or injected custom rule — to the matching records of a
`ProvGraph`'s transformed document, with a git-style match-set / not-found /
ambiguity contract. Stage 2 of the lineage sequence (`docs/research/lineage-direction.md`).
