...
- [x] 1.1 Create `packages/tsprov/scripts/browser-entry.ts` importing both barrels and
  assigning the flat merge to `globalThis.tsprov`, with WHY comments (scripts/ placement,
  flat-merge losslessness, spread order)
- [x] 1.2 Add `build:browser` to `packages/tsprov/package.json` using
...
- [x] 1.3 Run `bun run build` in `packages/tsprov` and confirm
...
- [x] 2.1 Extend `packages/tsprov/scripts/smoke.mjs` with a browser-bundle section: run
...
- [x] 2.2 Run `bun run smoke` and confirm the new section passes; then verify the failure
  path: temporarily move `dist/browser/tsprov.min.js` aside, assert `bun run smoke`
  exits non-zero, restore the file

## 3. Documentation

- [x] 3.1 Add a CHANGELOG.md Unreleased "Added" entry for the browser artifact (path,
  global name, consumer loading pattern)
- [x] 3.2 Check README.md's install/usage section only: if it enumerates the shipped
